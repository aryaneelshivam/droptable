        ...
        if uploaded_file is not None:
            ...
            for chart_type in chart_types:
                ...
                if chart_type == "Area Chart":
                    ...
                    y_axis2 = st.selectbox("Select for Line Chart - Y", df.columns, key=f"line_y_{chart_type}", index=None)
                    fig = px.line(df, x=x_axis1, y=y_axis2, width=1240)
                    st.plotly_chart(fig)
                elif chart_type == "Scatter Plot":
                    st.write("Select X-axis and Y-axis for Scatter Plot Chart")
                    x_axis3 = st.selectbox("Select for Scatter Plot - X", df.columns, key=f"scatter_x_{chart_type}", index=None)
                    y_axis3 = st.selectbox("Select for Scatter Plot - Y", df.columns, key=f"scatter_y_{chart_type}", index=None)
                    fig = px.scatter(df, x=x_axis3, y=y_axis3, width=1240)
                    st.plotly_chart(fig)
                elif chart_type == "Pie Chart":
                    selected_column = st.selectbox("Select Column for Pie Chart", df.columns, key=f"pie_column_{chart_type}", index=None)
                    ...
                    x_axis4 = st.selectbox("Select for Bubble Chart - X", df.columns, key=f"bubble_x_{chart_type}", index=None)
                    y_axis4 = st.selectbox("Select for Bubble Chart - Y", df.columns, key=f"bubble_y_{chart_type}", index=None)
                    size_column = st.selectbox("Select Size Column", df.columns, key=f"bubble_size_{chart_type}", index=None)
                    ...
                    hierarchy_column = st.selectbox("Select Hierarchy Column", df.columns, key=f"sunburst_hierarchy_{chart_type}", index=None)
                    values_column = st.selectbox("Select Values Column", df.columns, key=f"sunburst_values_{chart_type}", index=None)
                    ...
                    x_axis5 = st.selectbox("Select for Dot Plot Chart - X", df.columns, key=f"dot_x_{chart_type}", index=None)
            ...
            with tab2:
                ...
                def get_pyg_html(df: pd.DataFrame) -> str:
                    ...
                    html = get_streamlit_html(df, theme="light", use_kernel_calc=True, debug=False)
                    return html
